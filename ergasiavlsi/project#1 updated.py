...
def quine_mccluskey(minterms, dont_cares, num_vars):
    def find_prime_implicants(minterms, dont_cares, num_vars):
        ...
        terms = [bin(term)[2:].zfill(num_vars) for term in terms]
        ...
        while True:
            ...
            for term1 in terms:
                for term2 in terms:
                    ...
                    if combined_term:
                        new_implicants.add(combined_term)
                        used.add(term1)
                        used.add(term2)
            prime_implicants -= used
            if not new_implicants:
                break
            terms = list(new_implicants)
    ...
    def find_essential_prime_implicants(prime_implicants, minterms):
        ...
        minterm_bin = [bin(m)[2:].zfill(num_vars) for m in minterms]
        essential_pis = []
        for prime in prime_implicants:
            covers = [m for m in minterm_bin if is_covered(prime, m)]
            if len(covers) == 1:
                essential_pis.append(prime)
...
def minimize_expression(minterms, dont_cares, num_vars):
    if num_vars == 2:
        ...
        return "Quine-McCluskey: " + quine_mccluskey(minterms, dont_cares, num_vars)
